...
char *read_until_null_from_stdin() {
    ...
    int c;
    int i = 0;
    while ((c = getchar()) != EOF && c != '\0' && i < MAX_LINE_LENGTH - 1) {
        line[i++] = c;
    }
    line[i] = '\0';
    if (i == 0) {
...
char *read_until_null_from_file(FILE *file_handle) {
    ...
    int c;
    int i = 0;
    while ((c = fgetc(file_handle)) != EOF && c != '\0' && i < MAX_LINE_LENGTH - 1) {
        line[i++] = c;
    }
    line[i] = '\0';
    if (i == 0) {
...
void stat_file(FILE *file_handle) {
    ...
    while (1) {
        ...
        if (bytes_read < 0) {
            print_error("Error: reading file failed.");
        }
        if (bytes_read == 0) {
            break;
        }
        ...
        while (char_iter < total_bytes) {
            ...
            if (byte < 0x80) {
                ...
                global_chars_count++;
                wchar_t wc;
                int len = mbtowc(&wc, &buffer[char_iter], 2);
                if (len > 0) {
                ...
                global_chars_count++;
                wchar_t wc;
                int len = mbtowc(&wc, &buffer[char_iter], 3);
                if (len > 0) {
                    current_line_length += wcwidth(wc);
                }
                if (prev_space) {
                ...
                global_chars_count++;
                wchar_t wc;
                int len = mbtowc(&wc, &buffer[char_iter], 4);
                if (len > 0) {
                    current_line_length += wcwidth(wc);
                }
                if (prev_space) {
...
void print_statistics(int lines, int words, int chars, int bytes, int max_line_length, char *filename) {
    ...
    if (options_max_line_len) {
        if (first_field) {
            sprintf(output_string, "%d", max_line_length);
            first_field = false;
        } else {
